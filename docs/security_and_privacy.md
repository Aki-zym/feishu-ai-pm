...
- schema v11 的 batch 只持久化 owner/account scope、opaque batch/group key、内部 revision 引用、hash、primary/shared 关系和安全业务结果。普通主人决定 DTO 不返回 batch id、receipt、sender/chat/message 技术 ID、内部 revision、正文、candidate_key、prompt、模型推理、内部评分或原始 `last_error`；失败只投影固定布尔 `last_attempt_failed`，具体限长脱敏诊断保留在内部。append 选项只显示安全不可执行意图。每个 decision 的 options 使用单一 canonical stored projection，并在插件、HTTP、服务层和 SQLite CHECK 对齐到 10000 个 SQLite 文本字符；超限以 400 整批零业务写入。decision request id 在 owner/account scope 内唯一，版本冲突、跨账号访问、旧 revision、重复/漏覆盖和共享关系越权均 fail-closed。主人解决失败先回滚业务事务，再只记录内部限长脱敏 `last_error` 并保持 pending；取消和 append 拒绝均不消费来源，revision 更新则主动 supersede 旧 pending decision。
