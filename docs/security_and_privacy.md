...
- Cindy trusted-source schema v9 的 `source_receipt` 只证明来源经当前已认证的 Cindy 本机插件入口落库，不证明主机级 MCP provenance。owner scope、连接账号锚点和 receipt 密钥由 Bearer 认证上下文派生，body 自报字段由 strict schema 拒绝。receipt 至少使用 32 字节服务端随机 nonce，经 keyed derivation 形成不可预测 base64url 句柄；SQLite 只保存 nonce 与 receipt SHA-256 摘要，不把 receipt、稳定 message ID、外部 ID 或内部来源 ID 放进普通 DTO、普通日志或主人日常页面。未知、跨账号、旧 revision、superseded、revoked、invalid 或 legacy receipt 均 fail-closed。保存请求、来源 revision、关系图和决定分别在 SQLite 事务中原子处理；保存成功后的 Agent 中断不会回滚来源，决定失败不会删除已保存来源。
