...
- schema v10 不持久化原始 sender/chat/thread 技术 ID；服务端按 owner/account 域分离生成内部引用。display name 仅作为“MCP 回显后安全清洗的显示名”，不是法定身份认证；非字符串、对象、数组、空值、控制/Bidi 污染、字符串化对象占位及 `ou_`/`oc_` 技术 ID 一律安全清洗或回退“需求方”。清洗先 NFC，移除控制/Bidi 后再次 NFC，避免过滤后暴露的分解组合序列以非 canonical 形式落库；结果按完整 grapheme 截到最多 80 个且不超过 160 UTF-16 单元，因此同时满足 SQLite、候选和私人来源 DTO 的既有边界。只有主人私有候选/任务来源投影可显示该名称；公共来源 DTO、普通日志和诊断继续不含姓名、技术 ID、receipt 或正文。关系跨 account、chat、thread、owner 或失效 revision 时整批回滚。无 thread/reply 的批次还必须含明确主人事实并限制为 20 条/60 分钟；thread/reply 限 100 条/4 小时。
- schema v11 的 batch 只持久化 owner/account scope、opaque batch/group key、内部 revision 引用、hash、primary/shared 关系和安全业务结果。普通主人决定 DTO 不返回 batch id、receipt、sender/chat/message 技术 ID、内部 revision、正文、candidate_key、prompt、模型推理、内部评分或原始 `last_error`；失败只投影固定布尔 `last_attempt_failed`，具体限长脱敏诊断保留在内部。每个 decision 的 options 使用单一 canonical stored projection，并在插件、HTTP、服务层和 SQLite CHECK 对齐到 10000 个 SQLite 文本字符；超限以 400 整批零业务写入。decision request id 在 owner/account scope 内唯一，版本冲突、跨账号访问、旧 revision、重复/漏覆盖和共享关系越权均 fail-closed。主人解决失败先回滚业务事务，再只记录内部限长脱敏 `last_error` 并保持 pending；取消不消费来源，revision 更新则主动 supersede 旧 pending decision。
- schema v12 的 candidate key、conversation key 和 cursor 都使用 owner/account/receipt-secret 域分离 HMAC；普通上下文和 owner-decision UI 不返回内部 candidate/source/batch/group ID、receipt 或 raw error。append 只消费同 batch、同 primary group、current 且未消费的来源，`shared_context`、skip/needs_owner、跨账号和旧 revision 均 fail-closed。candidate CAS、来源 consumption 唯一约束、字段 patch/evidence、审计、owner decision resolved 和幂等响应共用一个事务；冲突或故障不留下部分来源关系或处理状态。新 owner-decision append option 只把 Agent 声明的 evidence 保存为内部 source-order 索引，普通 DTO 不投影；v11 旧 pending option 缺少显式索引时仅按其原合同兼容为全组 evidence。
