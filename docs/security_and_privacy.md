...
- schema v11 的 batch 只持久化 owner/account scope、opaque batch/group key、内部 revision 引用、hash、primary/shared 关系和安全业务结果。主人决定 DTO 不返回 receipt、sender/chat/message 技术 ID、正文、candidate_key、prompt、模型推理或内部评分；append 选项只显示安全候选标题并标记不可执行。decision request id 在 owner/account scope 内唯一，版本冲突、跨账号访问、旧 revision、重复/漏覆盖和共享关系越权均 fail-closed。主人解决失败先回滚业务事务，再只记录限长脱敏 `last_error` 并保持 pending；取消不消费来源，revision 更新则主动 supersede 旧 pending decision。
