...
|Aily staging inbox：`aily_summary_inbox`|TooManyTasks 官方 SDK 返回的窗口派生摘要；进入 `source_event` 前不属于任务来源真账|ready/claimed/retry_waiting 保留至处理；completed/failed 正文保留 30 天|停止自动扫描不删除已生成摘要；撤销 Aily 授权阻止新生成，不伪装成已处理|硬删除清除摘要正文、claim、错误分类和窗口记录|与主库同备份；claim token 只以 SHA-256 存储，原 token 只在插件内存中短暂存在|
...
- Aily inbox 的领取能力使用随机 claim token，SQLite 只保存 token hash；原 token 不进入 Cindy Prompt、模型最终文本、日志或任务来源。领取租约固定 10 分钟，失败只保存受控错误码并按 1、2、5、10、20 分钟退避，五次后终止为 failed。服务端只有在 `submit_intake` 同一事务验证窗口、摘要 hash、Agent、生成时间、租约和 claim token 后才允许标记 completed。
...
- Agent 首次部署也只允许使用当前用户自己新建的飞书应用和 Aily Agent。App Secret 从用户已登录的官方后台进入 Agent 当前进程内存，再通过 stdin 写入 TooManyTasks 的本机 AES-256-GCM 凭证库；不进入 GitHub、命令行参数、shell 历史、普通配置文件、聊天消息或诊断输出。无法自动完成的登录、管理员审批和 OAuth 允许仍保持在官方页面。
