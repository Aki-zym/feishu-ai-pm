...
> 2026-08-24 Cindy cross-window candidate append（阶段：Windows 桌面成熟化）：连续 schema v12 把 `get_pm_tasks` 演进为 owner/account 隔离的 `get_pm_context`，默认各返回最近 20 个正式任务和可追加候选、每类最多 50，并以加密且绑定 owner/account/kind 的 opaque cursor、稳定排序、最长 160 字符的确定性标题/短摘要子串过滤和 current receipts 派生的 conversation scope 限制回读。Cindy Agent 仍负责判断同目标追加、不同目标新建和歧义 `needs_owner`；服务端不做语义匹配或自动挑候选。`append_candidate` 只接受同 batch、同 primary group、current 且未消费的 receipts，对 pending/snoozed 候选执行 version CAS，并在一个 SQLite 事务中提交来源消费、候选来源、逐字段 evidence、字段 patch、candidate version、幂等响应、审计和 owner decision resolved。新 owner-decision append 选项会把 Agent 声明的 evidence 转成内部 source-order 索引持久化，普通 DTO/UI 不返回 candidate/source/batch/group 技术标识或原始错误；v11 旧 append 意图在迁移时只从受信任内部候选引用回填 candidate version，并把该 decision 的 current receipts 固化为明确 evidence 后才可恢复。历史 effective group key 被确定性迁为 collision-free internal key，v11→v12 迁移失败仍阻止启动并保持完整 v11。bundle 固定使用 esbuild 0.25.9，并以连续生成 hash、LF 和 clean committed HEAD 零 diff 作为门禁。当前 `product-source-sha256-v1` 指纹为 `66031d3ec0cd58c3a670be6e938fc7dc0e62180aaa7084f1d103967be25456ad`，纳入 `85` 个文件；验证记录为 `VER-LOCAL20260824-004-CINDY-APPEND-L2-20260824`。证据仅 synthetic/local，不证明真实 Cindy 模型的跨窗口归属质量、真实飞书租户或生产规模。

> 2026-08-24 Cindy grouped-batch privacy/replay follow-up：普通 owner-decision 投影不再返回内部 candidate id；batch 同 payload 重放固定返回首次持久化的安全响应，仅将 `duplicate` 置为 true，不混入之后的 owner-decision 实时状态。当时 `product-source-sha256-v1` 指纹为 `82b039e9da8fd011f42c8d8baf237269830516640489c98e3f0a5785b3ca9b41`，纳入 `85` 个文件。

> 2026-08-24 Cindy grouped-batch candidate（阶段：Windows 桌面成熟化）：连续 schema v11 将一次 Cindy 判断固定为 owner/account-bound `batch_id`、完整 receipt snapshot 和精确 revision set。每条来源恰有一个 `group / skip / needs_owner` primary disposition；group 的 anchor 与字段 evidence 角色互斥，其他 primary group 的来源只能通过唯一 `shared_context` 关系作为非归属背景。Cindy 只在已保存 snapshot 内按共同对象、目标和交付物完成语义分组；语义不得扩大回读范围或全局拉取会话，服务端和第二套 Runtime 不聚类。相同 canonical batch 重放复用原结果，异 payload 冲突；任一 receipt/CAS/结构失败使候选、任务、审计和处理结论整批回滚。`needs_owner` 普通 DTO 隐藏 batch/internal revision/raw error，只给安全摘要和固定失败布尔；canonical options 聚合上限与 SQLite CHECK 统一为 10000 字符。schema v11 首期只持久化 append 意图，成功路径由上方 schema v12 合同开放。磁盘 SQLite 双连接测试覆盖高 revision 先胜导致旧 batch 零写入，以及 batch 先胜后更高 revision 正常进入下一轮。v10→v11 迁移失败会阻止启动并恢复完整 v10。本轮 `product-source-sha256-v1` / `apps-workspace-default-include-v1` 指纹为 `4c4343a7c78c7bae595a6e5e8380ce617e5989876227e60250ae3782d6cfcacd`，纳入 `85` 个文件；验证记录为 `VER-LOCAL20260824-003-CINDY-GROUPED-BATCH-L2-20260824`。证据仅 synthetic/local，不证明真实 Cindy 模型的长期分组质量、真实飞书租户或生产并发。

> 2026-08-24 Cindy owner-context candidate（阶段：Windows 桌面成熟化）：在 trusted-source 两阶段合同上，连续 schema v10 为每个不可变 revision 保存 owner/account 隔离的 sender/chat/thread 内部引用、安全 display name、mention、主人发送、消息类型和主人白名单 reaction。display name 在移除控制/Bidi 后再次 NFC，再逐个完整 grapheme 截到最多 80 个且不超过最严格下游的 160 UTF-16 单元；对象占位和 `ou_`/`oc_` 技术 ID 回退“需求方”。正文不能声明这些事实；reply/thread 必须同账号、同 chat、兼容 thread，每个 thread/reply 连通上下文连同跨批 current receipt 祖先独立受 100 条/4 小时限制；无关系上下文仅限同 chat、包含明确主人证据并受 20 条/60 分钟限制。Cindy Agent 仍独立判断相关性和 `skip/create_candidate`，服务端不新增语义分类 Runtime；主人证据也不自动完成、排期或承诺。主人私有候选与任务来源可显示安全姓名，公共来源 DTO、日志和诊断继续隐藏技术 ID、receipt 与正文。v9→v10 失败会阻止启动并恢复完整 v9。本轮 `product-source-sha256-v1` / `apps-workspace-default-include-v1` 指纹为 `067062679e06ea577df8095f841444a6476a24ac7b3cf70dac27874d136ee611`，纳入 `84` 个文件；验证记录为 `VER-LOCAL20260824-002-CINDY-CONTEXT-L2-20260824`。证据仍仅 synthetic/local，不证明真实飞书租户、reaction/thread 权限或 MCP provenance attestation。
