...
> 2026-08-24 Cindy grouped-batch candidate（阶段：Windows 桌面成熟化）：连续 schema v11 将一次 Cindy 判断固定为 owner/account-bound `batch_id`、完整 receipt snapshot 和精确 revision set。每条来源恰有一个 `group / skip / needs_owner` primary disposition；group 的 anchor 与字段 evidence 角色互斥，其他 primary group 的来源只能通过唯一 `shared_context` 关系作为非归属背景。服务端只校验覆盖、scope、current revision、幂等、CAS 和事务，不按同人、同群、时间或文本自动聚类；候选标题、摘要和动作由 Cindy 生成并经过安全叙述门禁。相同 canonical batch 重放复用原结果，异 payload 冲突；任一 receipt/CAS/结构失败使候选、任务、审计和处理结论整批回滚。`needs_owner` 进入主人私有有界队列，可按 version 执行 `skip/create_candidate` 或取消；append 意图在 004 前保持 pending，不消费来源。v10→v11 迁移失败会阻止启动并恢复完整 v10。本轮 `product-source-sha256-v1` / `apps-workspace-default-include-v1` 指纹为 `62f2e79cc617eb726409b8c8f8c9da254b58c33f0acdc68bd9bcac58e6c31d89`，纳入 `85` 个文件；验证记录为 `VER-LOCAL20260824-003-CINDY-GROUPED-BATCH-L2-20260824`。证据仅 synthetic/local，不证明真实 Cindy 模型的长期分组质量、真实飞书租户或生产并发。
