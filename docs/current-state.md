...
> 2026-08-24 Cindy owner-context candidate（阶段：Windows 桌面成熟化）：在 trusted-source 两阶段合同上，连续 schema v10 为每个不可变 revision 保存 owner/account 隔离的 sender/chat/thread 内部引用、安全 display name、mention、主人发送、消息类型和主人白名单 reaction。正文不能声明这些事实；reply/thread 必须同账号、同 chat、兼容 thread，每个 thread/reply 连通上下文连同跨批 current receipt 祖先独立受 100 条/4 小时限制；无关系上下文仅从同 chat 的明确主人证据开始，受 20 条/60 分钟限制。Cindy Agent 仍独立判断相关性和 `skip/create_candidate`，服务端不新增语义分类 Runtime；主人证据也不自动完成、排期或承诺。主人私有候选与任务来源可显示安全姓名，公共来源 DTO、日志和诊断继续隐藏技术 ID、receipt 与正文。v9→v10 失败会阻止启动并恢复完整 v9。本轮 `product-source-sha256-v1` / `apps-workspace-default-include-v1` 指纹为 `2e724477cb3e0fdab54e79ea5f08aac48dd410baeb09e96349e5acf678520ab5`，纳入 `84` 个文件；验证记录为 `VER-LOCAL20260824-002-CINDY-CONTEXT-L2-20260824`。证据仍仅 synthetic/local，不证明真实飞书租户、reaction/thread 权限或 MCP provenance attestation。
