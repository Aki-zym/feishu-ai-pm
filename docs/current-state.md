...
> 2026-08-24 Cindy cross-window candidate append（阶段：Windows 桌面成熟化）：连续 schema v12 把 `get_pm_tasks` 演进为 owner/account 隔离的 `get_pm_context`，默认各返回最近 20 个正式任务和可追加候选、每类最多 50，并以 opaque cursor、稳定排序、最长 160 字符的确定性标题/短摘要子串过滤和 current receipts 派生的 conversation scope 限制回读。Cindy Agent 仍负责判断同目标追加、不同目标新建和歧义 `needs_owner`；服务端不做语义匹配或自动挑候选。`append_candidate` 只接受同 batch、同 primary group、current 且未消费的 receipts，对 pending/snoozed 候选执行 version CAS，并在一个 SQLite 事务中提交来源消费、候选来源、逐字段 evidence、字段 patch、candidate version、幂等响应、审计和 owner decision resolved。新 owner-decision append 选项会把 Agent 声明的 evidence 转成内部 source-order 索引持久化，普通 DTO/UI 不返回 candidate/source/batch/group 技术标识；v11 旧 append 意图保留原先隐式全组 evidence 语义。v11→v12 迁移失败阻止启动并保持完整 v11。当前 `product-source-sha256-v1` 指纹为 `e22347bf862d87773a026038537d9ba0ade538f3df2322c02683f8d1630d720b`，纳入 `85` 个文件；验证记录为 `VER-LOCAL20260824-004-CINDY-APPEND-L2-20260824`。证据仅 synthetic/local，不证明真实 Cindy 模型的跨窗口归属质量、真实飞书租户或生产规模。
