...
- TooManyTasks 的 Aily 派生摘要必须使用 source_kind=aily_summary、窗口稳定 source_key、derivedEvidence=true 和 limited completeness；普通 intake 与 empty_window 使用各自明确合同。Aily 或 Cindy 失败、服务端冲突或缺少成功回执不得推进窗口游标；update_task 必须携带本地任务快照中的 expected_version 并由 SQLite CAS 事务裁决。
