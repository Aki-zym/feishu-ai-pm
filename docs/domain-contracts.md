...
- TooManyTasks 的 Aily 派生摘要必须先进入 schema v9 aily_summary_inbox staging 队列，再由 Cindy intake 提升为 source_kind=aily_summary、窗口稳定 source_key、derivedEvidence=true 和 limited completeness。Aily 成功持久化摘要或空窗后才推进独立扫描游标；Aily 失败不得推进。Cindy 失败只推进 inbox 的 retry_waiting/failed 状态，不能删除摘要或阻塞后续扫描窗口。submit_intake 必须在同一 SQLite 事务校验窗口、摘要 hash、Agent、生成时间、10 分钟租约和 claim token，并与来源/候选/任务写入一起标记 completed；update_task 继续使用 expected_version CAS。
