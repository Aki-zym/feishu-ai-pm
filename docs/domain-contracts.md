...
- Cindy owner-context schema v10 在不可变 source revision 保存 owner/account 隔离的 sender/chat/thread 内部引用、安全 display name、mentioned_owner、sender_is_owner、message_type 与主人白名单 reaction。原始技术 ID 不进入普通 DTO、日志或诊断；display name 按 NFC、去控制/Bidi、折叠空白、最多 80 个可见字符处理，非字符串或空值回退‘需求方’。身份、mention、reply/thread、reaction 不从正文推断。关系仅接受同认证上下文、同 owner、同 chat、兼容 thread 的 current receipt 或同批 client_ref；未知、跨账号、失效、成环或技术 ID 注入整批零写入。每个 thread/reply 连通上下文连同跨批 current receipt 祖先独立受最多 100 条/4 小时限制；无关系上下文仅限同 chat、从明确主人证据开始、最多 20 条/60 分钟。服务端不实现语义相关性分类，主人事实不自动创建、完成、排期或承诺。v9→v10 失败阻止启动并保持完整 v9。
