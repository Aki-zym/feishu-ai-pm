...
import type { DurableEventReceipt, FeishuAdapter, IntegrationCheck, WorkspaceReferenceAdapter } from '../integration-contracts.js';
...
/**
 * Cindy resident runtime deliberately has no classifier or live Feishu
 * adapter. Source rows arrive through the guarded Cindy intake contract.
 */
...
export type CindyAdapterSet = {
  feishu: FeishuAdapter;
  workspace: WorkspaceReferenceAdapter;
};

export function createCindyAdapters(_config: AppConfig): CindyAdapterSet {
  return {
    feishu: new DisabledFeishuAdapter(),
    workspace: new ReferenceOnlyWorkspaceAdapter(),
  };
}
