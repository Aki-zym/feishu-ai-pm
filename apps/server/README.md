...
Cindy 插件通过浏览器本机后台分两步入库：先把最小来源事实原子保存到 SQLite，并取得服务端签发的 opaque `source_receipt`；再只用 receipts 提交结构化判断。Bearer 只负责 HTTP 鉴权；稳定连接账号锚点与独立 receipt 密钥由插件首次启动时生成并保存，轮换 Bearer 不改变旧 receipt、来源身份或幂等记录。请求 body 不能自报 owner/account；正式任务更新继续执行版本 CAS。生产 `buildApp` 不注册 raw-source intake 或 seed 路由；旧 Feishu/模型扫描入口已停止导出。
